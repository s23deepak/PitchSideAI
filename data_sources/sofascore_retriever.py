"""
Sofascore Retriever — live stats, lineups, heatmaps, player ratings.

Uses Tavily/Exa search with sofascore.com domain filtering to extract
structured match data. Falls back to general web search.
"""
...
SOFASCORE_SEARCH_DOMAINS = ["sofascore.com"]


class SofascoreRetriever(BaseRetriever):
    def __init__(self, cache=None, search_service=None):
        super().__init__(
            source_name="sofascore",
            source_tier=get_source_tier("sofascore"),
    ...
    async def _do_fetch(
        ...
        include_domains = params.get("include_domains", SOFASCORE_SEARCH_DOMAINS)
